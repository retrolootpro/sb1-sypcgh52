...
import { Plus, ShoppingBag, ExternalLink, DollarSign } from 'lucide-react';
import {
  createEbayListing,
  updateEbayListing,
...
export function EbayTab({ employeeId, listings, onRefresh }: EbayTabProps) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ item_id: '', listed_price: '', listing_url: '', listing_id: '' });
      setCreateOpen(false);
      onRefresh();
    } catch {
  ...
  const handleStatusChange = async (listingId: string, newStatus: string) => {
    ...
    try {
      ...
      await updateEbayListing(listingId, updates as Parameters<typeof updateEbayListing>[1]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold">eBay Listings</h3>
      ...
      {listings.length === 0 ? (
        <div className="rounded-xl border border-border/40 bg-card/50 flex flex-col items-center justify-center py-12">
          ...
          <p className="text-sm text-muted-foreground mb-1">No eBay listings</p>
          <p className="text-xs text-muted-foreground/60">Add a listing to track eBay sales</p>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    {listing.listing_id && (
                      <span className="text-xs text-muted-foreground font-mono">#{listing.listing_id}</span>
                    )}
                  </div>
                </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>New eBay Listing</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-white/50">Listing ID (optional)</Label>
              <Input
