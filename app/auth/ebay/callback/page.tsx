...
export default function EbayCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus('error');
      setMessage('eBay authorization was cancelled or denied.');
      setTimeout(() => router.push('/settings?ebay=error'), 2500);
      return;
    }
    ...
    const redirectUri = typeof window !== 'undefined'
      ? localStorage.getItem('ebay_runame') || `${window.location.origin}/auth/ebay/callback`
      : '';
    ...
    (async () => {
      try {
        ...
        const { data, error: fnError } = await supabase.functions.invoke('ebay-auth', {
          body: { action: 'exchange', code, redirect_uri: redirectUri },
        });

        if (fnError || data?.error) {
          setStatus('error');
          setMessage(data?.error || fnError?.message || 'Failed to connect eBay account.');
          setTimeout(() => router.push('/settings?ebay=error'), 2500);
          return;
        }

        if (typeof window !== 'undefined') {
          localStorage.removeItem('ebay_runame');
        }

        setStatus('success');
        setMessage('eBay account connected successfully!');
        setTimeout(() => router.push('/settings?ebay=connected'), 2000);
      } catch (err: unknown) {
